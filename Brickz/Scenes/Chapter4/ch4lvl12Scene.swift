...
class ch4lvl12Scene: SKScene, SKPhysicsContactDelegate {
    ...
    var brickLives: Array = [9,9,9,9,9,9,9,9,9,9,9,9,9]
    ...
    var ch4lvl13Unlocked = UserDefaults.standard.bool(forKey: "ch4lvl13Unlocked")
    ...
    override func didMove(to view: SKView) {
        ...
        brick11.position = CGPoint(x: brick1.position.x - (brick1.size.width), y: brick1.position.y - (brick1.size.height))
        ...
        topSensor.position = CGPoint(x: self.frame.width / 2, y: brick7.position.y - (brick7.size.width / 2))
        ...
        bottomSensor.position = CGPoint(x: self.frame.width / 2, y: brick4.position.y + (brick4.size.width / 2))
        ...
        rightSensor.size = CGSize(width: 1, height: self.frame.height)
        rightSensor.position = CGPoint(x: brick4.position.x - (brick4.size.width / 2), y: self.frame.height / 2)
        rightSensor.zPosition = 1
        rightSensor.color = SKColor.clear
        rightSensor.name = "rightSensor"
        self.addChild(rightSensor)
        rightSensor.physicsBody = SKPhysicsBody(rectangleOf: rightSensor.size)
        rightSensor.physicsBody?.isDynamic = false
        rightSensor.physicsBody?.affectedByGravity = false
        rightSensor.physicsBody?.allowsRotation = false
        rightSensor.physicsBody?.friction = 0
        rightSensor.physicsBody?.restitution = 1
        rightSensor.physicsBody?.categoryBitMask = PhysicsCategory.Sensor
        rightSensor.physicsBody?.collisionBitMask = PhysicsCategory.Obstacle
        rightSensor.physicsBody?.contactTestBitMask = PhysicsCategory.Obstacle
        
        leftSensor.size = CGSize(width: 1, height: self.frame.height)
        leftSensor.position = CGPoint(x: brick10.position.x + (brick10.size.width / 2), y: self.frame.height / 2)
        leftSensor.zPosition = 1
        leftSensor.color = SKColor.clear
        leftSensor.name = "leftSensor"
        self.addChild(leftSensor)
        leftSensor.physicsBody = SKPhysicsBody(rectangleOf: leftSensor.size)
        leftSensor.physicsBody?.isDynamic = false
        leftSensor.physicsBody?.affectedByGravity = false
        leftSensor.physicsBody?.allowsRotation = false
        leftSensor.physicsBody?.friction = 0
        leftSensor.physicsBody?.restitution = 1
        leftSensor.physicsBody?.categoryBitMask = PhysicsCategory.Sensor
        leftSensor.physicsBody?.collisionBitMask = PhysicsCategory.Obstacle
        leftSensor.physicsBody?.contactTestBitMask = PhysicsCategory.Obstacle
        ...
        moveObstacleLeft = SKAction.moveTo(x: leftSensor.position.x, duration: 2)
        moveObstacleRight = SKAction.moveTo(x: rightSensor.position.x, duration: 2)
        
        obstacleTop.size = CGSize(width: self.frame.width / 8, height: self.frame.height / 40)
        obstacleTop.position = CGPoint(x: leftSensor.position.x + (obstacleTop.size.width * 0.6), y: brick10.position.y + (brick10.size.height / 3))
        ...
        obstacleBottom.size = CGSize(width: self.frame.width / 8, height: self.frame.height / 40)
        obstacleBottom.position = CGPoint(x: rightSensor.position.x - (obstacleBottom.size.width * 0.6), y: brick4.position.y - (brick4.size.height / 3))
        ...
        obstacleLeft.position = CGPoint(x: brick10.position.x - (brick1.size.width / 3), y: topSensor.position.y - (obstacleLeft.size.height * 0.6))
        ...
        obstacleRight.position = CGPoint(x: brick4.position.x + (brick4.size.width / 3), y: bottomSensor.position.y + (obstacleRight.size.height * 0.6))
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if(contact.bodyA.node != nil && contact.bodyB.node != nil) {
            ...
            if (firstBody.name == "obstacleTop") && (secondBody.name == "leftSensor") {
                movingObstacleHitSensor(Obstacle: firstBody, Sensor: secondBody)
            } else if (firstBody.name == "leftSensor") && (secondBody.name == "obstacleTop") {
                movingObstacleHitSensor(Obstacle: secondBody, Sensor: firstBody)
            }
            if (firstBody.name == "obstacleTop") && (secondBody.name == "rightSensor") {
                movingObstacleHitSensor(Obstacle: firstBody, Sensor: secondBody)
            } else if (firstBody.name == "rightSensor") && (secondBody.name == "obstacleTop") {
                movingObstacleHitSensor(Obstacle: secondBody, Sensor: firstBody)
            }
            if (firstBody.name == "obstacleBottom") && (secondBody.name == "leftSensor") {
                movingObstacleHitSensor(Obstacle: firstBody, Sensor: secondBody)
            } else if (firstBody.name == "leftSensor") && (secondBody.name == "obstacleBottom") {
                movingObstacleHitSensor(Obstacle: secondBody, Sensor: firstBody)
            }
            if (firstBody.name == "obstacleBottom") && (secondBody.name == "rightSensor") {
                movingObstacleHitSensor(Obstacle: firstBody, Sensor: secondBody)
            } else if (firstBody.name == "rightSensor") && (secondBody.name == "obstacleBottom") {
                movingObstacleHitSensor(Obstacle: secondBody, Sensor: firstBody)
            }
    ...
    func movingObstacleHitSensor(Obstacle: SKSpriteNode, Sensor: SKSpriteNode) {
        if Obstacle.name == "obstacleTop" && Sensor.name == "leftSensor" {
            obstacleTop.run(moveObstacleRight)
        }
        if Obstacle.name == "obstacleTop" && Sensor.name == "rightSensor" {
            obstacleTop.run(moveObstacleLeft)
        }
        if Obstacle.name == "obstacleBottom" && Sensor.name == "leftSensor" {
            obstacleBottom.run(moveObstacleRight)
        }
        if Obstacle.name == "obstacleBottom" && Sensor.name == "rightSensor" {
            obstacleBottom.run(moveObstacleLeft)
        }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if touchedNode.name == "nextBtn" {
                let scene = ch4lvl2Scene(size: CGSize(width: (view?.frame.width)!, height: (view?.frame.height)!))
                let transition = SKTransition.fade(withDuration: 0.5)
                scene.scaleMode = .aspectFill
                self.view?.presentScene(scene, transition: transition)
            }
    ...
    func ballHitBrick(Ball: SKSpriteNode, Brick: SKSpriteNode) {
        ...
        if brickCount == 0 {
            print("ch4lvl12 completed")
            gameOver = true
            
            if ch4lvl13Unlocked == false {
                continueString = "ch4lvl13"
                UserDefaults.standard.set("ch4lvl13", forKey: "continueString")
            }
            ch4lvl13Unlocked = true
            UserDefaults.standard.set(true, forKey: "ch4lvl13Unlocked")
            UserDefaults.standard.synchronize()
    ...
    func explosionRadiusHitBrick(Radius: SKSpriteNode, Brick: SKSpriteNode) {
        ...
        if brickCount == 0 {
            ...
            if ch4lvl13Unlocked == false {
                continueString = "ch4lvl13"
                UserDefaults.standard.set("ch3lvl13", forKey: "continueString")
            }
            ch4lvl13Unlocked = true
            UserDefaults.standard.set(true, forKey: "ch4lvl13Unlocked")
            UserDefaults.standard.synchronize()
