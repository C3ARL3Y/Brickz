...
class ch4lvl8Scene: SKScene, SKPhysicsContactDelegate {
    ...
    var brickLives: Array = [9,9,9,9,9,9,9,9,9]
    ...
    var ch4lvl9Unlocked = UserDefaults.standard.bool(forKey: "ch4lvl9Unlocked")
    ...
    var moveObstacle2 = SKAction()
    var moveObstacle3 = SKAction()
    ...
    override func didMove(to view: SKView) {
        ...
        brick3.position = CGPoint(x: brick1.position.x + (brick1.size.width), y: brick1.position.y + (brick1.size.height / 2))
        brick3.name = "brick3"
        brick3.zPosition = 1
        ...
        brick4.position = CGPoint(x: self.frame.width / 2, y: brick1.position.y - (brick1.size.height * 2))
        brick4.name = "brick4"
        brick4.zPosition = 1
        ...
        brick7.position = CGPoint(x: self.frame.width - (brick7.size.width * 0.6), y: brick6.position.y - (brick6.size.height * 2))
        brick7.name = "brick7"
        brick7.zPosition = 1
        ...
        brick9.position = CGPoint(x: brick8.position.x - (brick8.size.width), y: brick8.position.y + (brick8.size.width / 2))
        brick9.name = "brick9"
        brick9.zPosition = 1
        ...
        moveObstacle2 = SKAction.moveTo(x: self.frame.width, duration: 1.65)
        moveObstacle3 = SKAction.moveTo(x: self.frame.width, duration: 0.55)
        
        obstacle1.size = CGSize(width: self.frame.width / 6, height: self.frame.height / 40)
        obstacle1.position = CGPoint(x: 0 + (obstacle1.size.width * 0.6), y: brick2.position.y + (brick2.size.height))
        ...
        obstacle2.position = CGPoint(x: brick3.position.x + (brick3.size.width), y: brick1.position.y - (brick1.size.height))
        obstacle2.zPosition = 1
        obstacle2.color = SKColor.darkGray
        obstacle2.name = "obstacle2"
        self.addChild(obstacle2)
        obstacle2.run(moveObstacle2)
        ...
        obstacle3.position = CGPoint(x: brick6.position.x + (brick6.size.width), y: brick6.position.y - (brick6.size.height))
        obstacle3.zPosition = 1
        obstacle3.color = SKColor.darkGray
        obstacle3.name = "obstacle3"
        self.addChild(obstacle3)
        obstacle3.run(moveObstacle3)
        ...
        obstacle4.position = CGPoint(x: self.frame.width - (obstacle4.size.width * 0.6), y: brick8.position.y - (brick8.size.height))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if touchedNode.name ==  "retryBtn" {
                let scene = ch4lvl8Scene(size: CGSize(width: (view?.frame.width)!, height: (view?.frame.height)!))
                let transition = SKTransition.fade(withDuration: 0.5)
                scene.scaleMode = .aspectFill
                self.view?.presentScene(scene, transition: transition)
            }
            if touchedNode.name == "nextBtn" {
                let scene = ch4lvl9Scene(size: CGSize(width: (view?.frame.width)!, height: (view?.frame.height)!))
                let transition = SKTransition.fade(withDuration: 0.5)
                scene.scaleMode = .aspectFill
                self.view?.presentScene(scene, transition: transition)
            }
    ...
    func ballHitBrick(Ball: SKSpriteNode, Brick: SKSpriteNode) {
        ...
        if brickCount == 0 {
            print("ch4lvl8 completed")
            gameOver = true
            
            if ch4lvl9Unlocked == false {
                continueString = "ch4lvl9"
                UserDefaults.standard.set("ch4lvl9", forKey: "continueString")
            }
            ch4lvl9Unlocked = true
            UserDefaults.standard.set(true, forKey: "ch4lvl9Unlocked")
            UserDefaults.standard.synchronize()
